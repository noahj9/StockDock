...
class NewDocketForm(ModelForm): #model form for new docket
    ...
    Inks_2 = forms.ModelChoiceField(queryset=Ink.objects.all(), required = True, widget=forms.Select(attrs = {'class': 'form-control'}))
    Inks_3 = forms.ModelChoiceField(queryset=Ink.objects.all(), required = True, widget=forms.Select(attrs = {'class': 'form-control'}))
