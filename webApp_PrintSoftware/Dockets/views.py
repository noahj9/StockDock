...
def updateDocket(request, pk):
    # Use get_object_or_404 to handle DoesNotExist gracefully with a 404 response
...
def printDocket(req, pk):
    # Use get_object_or_404 to handle DoesNotExist gracefully with a 404 response
    # This prevents cascading failures when either the docket or contact doesn't exist
    docket = get_object_or_404(Docket, id=pk)
